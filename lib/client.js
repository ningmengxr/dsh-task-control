...
function CheckButton({ useSession, sessionId, cancelSession }) {
  ...
  const runningCalls = useSession((s2) => s2.runningCalls);
...
function EmergencyButton({ useSession, cancelSession }) {
  const s = useSettings();
  ...
  const doEmergency = async () => {
    ...
    try {
      ...
      cancelSession();
      setResult(killed > 0 ? `\u5DF2\u6025\u505C\uFF1A\u5F3A\u5236\u7EC8\u6B62\u4E86 ${killed} \u4E2A\u8FDB\u7A0B\uFF0C\u4EFB\u52A1\u5DF2\u505C\u6B62` : markers.length > 0 ? "\u5DF2\u53D1\u9001\u6025\u505C\u6307\u4EE4\uFF08\u672A\u5339\u914D\u5230\u53EF\u7EC8\u6B62\u7684\u8FDB\u7A0B\uFF0C\u53EF\u80FD\u5DF2\u81EA\u884C\u7ED3\u675F\uFF09" : "\u5DF2\u53D1\u9001\u6025\u505C\u6307\u4EE4\uFF08\u4EFB\u52A1\u6CA1\u6709\u53EF\u8BC6\u522B\u7684\u8FDB\u7A0B\uFF0C\u82E5\u4ECD\u5361\u4F4F\u53EF\u7A0D\u540E\u91CD\u8BD5\uFF09");
      setOpen(true);
    } finally {
      setBusy(false);
    }
