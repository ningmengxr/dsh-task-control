...
function useSnapshotSlice(mode, props) {
  const chat = mode === "chat" ? props.useChat((s) => s?.legacy, sliceEq) : void 0;
  const session = mode === "session" ? props.useSession((s) => ({ runningCalls: s?.runningCalls, nodes: s?.nodes }), sliceEq) : void 0;
  const slice = chat ?? session;
  return {
    runningCalls: slice?.runningCalls ?? [],
    nodes: slice?.nodes ?? []
  };
}
...
function CheckButton(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CheckButtonCore, { mode, ...props }, mode);
}
function CheckButtonCore({ mode, ...props }) {
  ...
  const checkPwsh = async (toolName, argsRaw, startTime) => {
    ...
    if (status.active) {
      ...
      } else if (status.fileSizeBytes >= 0 && lastPwshSample !== null && status.fileSizeBytes > lastPwshSample.fileSizeBytes) {
        const dt = (now - lastPwshSample.time) / 1e3;
...
function EmergencyButton(props) {
  const mode = snapshotMode(props);
  if (mode === null) return null;
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EmergencyButtonCore, { mode, ...props }, mode);
}
function EmergencyButtonCore({ mode, ...props }) {
...
async function fetchPwshStatus(marker, outPath) {
  try {
